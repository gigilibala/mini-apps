              ...
              YAML_Doc& doc)
{
  ...
  while (cur_imbalance < imbalance) {
    ...
    int shrink_info[8] = {shrink.first, shrink.second,
                        ...
                        local_box[Z][0], local_box[Z][1]};
#ifdef HAVE_MPI
    MPI_Bcast(&grow_info[0], 8, MPI_INT, max_proc, FTComm::get_instance()->get_world_comm());
    MPI_Bcast(&shrink_info[0], 8, MPI_INT, min_proc, FTComm::get_instance()->get_world_comm());
#endif
    ...
    MPI_Allreduce(&statusints, &globalstatus, 2, MPI_INT, MPI_SUM, FTComm::get_instance()->get_world_comm());
